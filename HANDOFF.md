...
- **Logo tagline alignment:** in the footer lockup the tagline sits centred under the wordmark. Both `Logo.tsx` children stretch to the wordmark's width (flex column), so a bare `text-center` isn't enough on its own — the tagline also carries a `pl` equal to its `tracking` value, which offsets the invisible gap `letter-spacing` adds after the last glyph. Without that compensation the text sits ~2px left of true centre; with it the measured offset is 0.03px. Keep the two values equal.
- **Email signature** lives in `email-signature/` (`threeeighty-signature.html` + a Ukrainian `README.md` with Gmail/Outlook/Apple Mail steps). It is table-based with inline styles only, and points at a **hosted** logo `public/email/threeeighty-logo.png` (421×128, shown at 210×64 = exactly 2× for retina) rather than embedding the image. That PNG was generated by rendering the lockup on a live page and screenshotting the element, so it matches `Logo.tsx` exactly — if the logo changes visually on the site, the PNG must be regenerated the same way. Don't delete the file or change the URL: every already-sent signature references it. The footer/contact email everywhere is `hello@threeeighty.eu` (the old `hello@rel1ve.eu` was replaced 2026-09-18).
